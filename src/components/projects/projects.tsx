    ...
    const targets = targetData.map(target => (
        ...
        key={target.tittle}>
            ...
            <div className="w-full flex justify-end project-target-button">
                <a href={target.url} target='_blank' className='flex gap-2 bg-[#0000001e] p-2 w-auto h-auto items-center justify-center rounded-[10px] no-underline text-white text-[0.9rem] mt-2 border border-[#ffffff2b] transition-all duration-1000 hover:bg-[#e7e7e749]'>
                    {target.button}
